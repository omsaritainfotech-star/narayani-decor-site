...
export default async function ProductPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <>
      <Seo title={product?.seo?.title || product?.title} description={product?.seo?.description} image={product?.seo?.og_image || product?.images?.[0]} type="product" productPrice={product?.price}
        breadcrumbs={[{name:'Home',item:'/'},{name:'Products',item:'/#products'},{name:product?.title,item:`/products/${product?.slug}`}]}
      />
      <section className="px-6 py-16 mx-auto max-w-5xl">
        ...
        <div className="mt-6"><RichText content={product.body || ""} /></div>
      </section>
    </>
  );
}
